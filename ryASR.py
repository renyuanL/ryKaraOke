...
# 修正了 執行外部程式時，會彈出Dos窗的問題
import rySpeech_recognition as sr 
...
# simple demo ....
#---------
def 聽寫(ryasr, 秒數= 10, dt= .1):

    ryasr.麥.等待第一圈錄音圓滿()
    ryasr.麥.等待第一圈錄音圓滿_wav()
    
    while ryasr.基頻音符們 == []: pass
    while ryasr.文們 == []: pass
    ...
    while t<T:
        ...
        print('t= {:.1f}: 能量= {}, 基頻音符= {}, 文= {}'.format(t, 能量, 基頻音符, 文))
        time.sleep(dt) # 1000 * .1 == 100
        t += dt

import turtle as tt

def 龜圖(ryasr, 秒數= 10, dt= .1):
    ...
    while t<T:
        ...
        基頻= ryasr.基頻音符們[-1][-2]
        文= ryasr.文們[-1][-1]
        tt.goto(int(t*1000)%1000-400,基頻-400)
        tt.stamp()
        tt.write(文)
        if int(t*1000)%2000<100: tt.clear()
        
        #time.sleep(dt) #tt 自己很慢，不用再睡覺了。
        t += dt
    
def wav聽寫(ryasr, 秒數= 10, dt= .1):
    
    ryasr.麥.等待第一圈錄音圓滿()
    ryasr.麥.等待第一圈錄音圓滿_wav()
    
    while ryasr.wav基頻音符們 == []: pass
    #while ryasr.文們 == []: pass
...
if __name__=='__main__':
    ...
    #聽寫(x,100)
    th1= threading.Thread(target= 聽寫, args=[x], kwargs={'秒數': 100})
    #th1.start()
    
    #龜圖(x, 100)
    th2= threading.Thread(target= 龜圖, args=[x], kwargs={'秒數': 100})
    #th2.start()
    
    print('聽寫(x, 秒數= 10)')
    print('龜圖(x, 秒數= 100)')
    
    print('wav聽寫(ryasr, 秒數= 10, dt= .1)')
