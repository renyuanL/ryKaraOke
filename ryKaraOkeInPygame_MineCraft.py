...
class Ry頻譜類:
    ...
    def 頻譜線程(我):
        ...
        def 取音訊且顯示頻譜於幕(我, 鍵盤= None):
            ...
            for xx in range(16):
                xxx= xx*w//16
                c= 255-(xx*256*4//16)%256  # 4 個一循環，灰階
...
def usingMineCraft(ry麥, ry頻譜):
    ...
    pos0= mc.player.getPos()
    dr0=  mc.player.getDirection()

    while True:
       
       pos= pos0 #mc.player.getPos()
       dr=  dr0  #mc.player.getDirection()

       
       f0, f1= getF0(ry麥, ry頻譜)
       print('f0, f1= {:.0f} {:.0f}, pos={}, dr={}'.format(f0,f1, pos, dr))
       ...
       mc.setBlock(pL[-1], block.GOLD_BLOCK)
...
if __name__ == '__main__':
    ...
    #usingMineCraft(ry麥, ry頻譜)
    mineCraft線= threading.Thread(target= usingMineCraft, 
                                  args= (ry麥, ry頻譜))  
    mineCraft線.start()
